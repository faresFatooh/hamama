...
public class RequestDriverActivity extends AppCompatActivity {
    ...
    private LatLng mDriverFoundLatLong;
    ...
    private void getClosesDriver(){
        mGeofireProvider.getActiveDriver(mOriginLatLng, mRadius).addGeoQueryEventListener(new GeoQueryEventListener() {
            @Override
            public void onKeyEntered(String key, GeoLocation location) {
                if (!mDriverFound){
                     mDriverFound = true;
                     mIdDriverFound = key;
                     mDriverFoundLatLong = new LatLng(location.latitude,location.longitude);
   ...
   private void createClientBooking(){
       mGoogleApiProvider.getDirections(mOriginLatLng, mDriverFoundLatLong).enqueue(new Callback<String>() {
           @Override
    ...
    private void sendNotificationCancel() {
        mTokenProvider.getTokens(mIdDriverFound).addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
                            ...
                            if (response.body() != null) {
                                if (response.body().getSuccess() == 1) {
                                  ...
                                  //  Toast.makeText(RequestDriverActivity.this, "La notificacion se  ha enviado correctamente", Toast.LENGTH_SHORT).show();
                                    ...
                                    Toast.makeText(RequestDriverActivity.this, "No se pudo mandar la notificacion", Toast.LENGTH_SHORT).show();
                                }
                            } else {
                                Toast.makeText(RequestDriverActivity.this, "No se pudo mandar la notificacion", Toast.LENGTH_SHORT).show();
    ...
    private void chekStatusClientBooking() {
